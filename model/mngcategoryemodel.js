console.log("mngcategorymodel.js is running");
...
class Category {
    ...
    static async getAll() {
        try {
            ...
            const [data] = await conn.query('SELECT * FROM tbl_categories'); // Fetch data
    ...
    static async createCategory(userData) {
        ...
        try {
            const conn = await db.getConnection(); // Get DB connection
            const sql = `INSERT INTO tbl_categories (ctr_name ,ctr_code , active_status) 
                         VALUES (?, ? ,?)`;
    ...
    static async dltcategory(id) {
        try {
            const conn = await db.getConnection();
            const query = `DELETE FROM tbl_categories WHERE nid = ?`;
